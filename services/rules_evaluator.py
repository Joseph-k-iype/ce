...
class RulesEvaluator:
    ...
    def evaluate(
        ...
        if not country_matched_rules and not triggered_rules:
            # Build graph with whatever we have (Phase 0 only)
            evaluation_graph = self._build_evaluation_graph(context, triggered_rules)
        ...
        # ── PHASE 2: Build Evaluation Graph (after all rules collected) ──
        evaluation_graph = self._build_evaluation_graph(context, triggered_rules)

        # ── PHASE 3: Search for precedent cases ───────────────────────
    ...
    def _match_graph_linked_attributes(
        ...
        if linked_purposes:
            ...
            if matched_norm:
                ...
                current_matches["PurposeOfProcessing"] = matched_originals
    ...
    def _build_evaluation_graph(
        self, context: EvaluationContext, triggered_rules: List[TriggeredRule]
    ) -> EvaluationGraph:
        """Build a graph representing the evaluation path from origin to outcome."""
        nodes = []
        edges = []
        seen_node_ids = set()
        edge_counter = 0
        ...
        # 1. Origin Country
        origin_id = f"country_{context.origin_country.replace(' ', '_')}_origin"
        nodes.append(EvaluationNode(
            id=origin_id,
            type="countryNode",
            data={
                "label": context.origin_country,
                "nodeType": "Country",
                "lane": "originCountry"
            }
        ))
        seen_node_ids.add(origin_id)

        # 2. Receiving Country
        receiving_id = f"country_{context.receiving_country.replace(' ', '_')}_recv"
        nodes.append(EvaluationNode(
            id=receiving_id,
            type="countryNode",
            data={
                "label": context.receiving_country,
                "nodeType": "Country",
                "lane": "receivingCountry"
            }
        ))
        seen_node_ids.add(receiving_id)

        # 3. Triggered Rules and their Paths
        for rule in triggered_rules:
            rule_node_id = f"rule_{rule.rule_id}"
            if rule_node_id not in seen_node_ids:
                nodes.append(EvaluationNode(
                    id=rule_node_id,
                    type="ruleNode",
                    data={
                ...
                seen_node_ids.add(rule_node_id)

            # Country Trigger Edges
            edges.append(EvaluationEdge(
                id=f"ev_e_{edge_counter}",
                source=origin_id,
                target=rule_node_id,
            ...
            # 4. Required Assessments (PIA/TIA/HRPR) as Duty nodes
            if rule.permissions:
                for perm in rule.permissions:
                    perm_node_id = f"permission_{perm.name.replace(' ', '_')}" if perm.name else None
                    if perm_node_id and perm_node_id not in seen_node_ids:
                        nodes.append(EvaluationNode(
                            id=perm_node_id,
                            type="caseModuleNode",
                            data={
                        ...
                        seen_node_ids.add(perm_node_id)
                    ...
                    if perm.duties:
                        for duty in perm.duties:
                            duty_node_id = f"duty_{duty.name.replace(' ', '_')}" if duty.name else None
                            if duty_node_id and duty_node_id not in seen_node_ids:
                                nodes.append(EvaluationNode(
                                    id=duty_node_id,
                                    type="caseModuleNode",
                                    data={
                                        ...
                                        "nodeType": "Duty",
                                        "lane": "duty",
                                        "module": duty.module if hasattr(duty, 'module') else None,
                                        "value": duty.value if hasattr(duty, 'value') else None,
                                    }
                                ))
                                seen_node_ids.add(duty_node_id)

                            if duty_node_id and perm_node_id:
                                edges.append(EvaluationEdge(
                                    id=f"ev_e_{edge_counter}",
                                    source=perm_node_id,
                                    target=duty_node_id,
                                    data={"relationship": "CAN_HAVE_DUTY"}
                                ))
                                edge_counter += 1

            # 5. Triggered Mapping Nodes (DataCategory, Purpose, etc.)
            mappings = context.triggered_node_mappings.get(rule.rule_id, {})
            for label, names in mappings.items():
                lane = get_lane_for_label(label)
                rf_type = get_rf_type_for_label(label)

                for name in names:
                    mapped_node_id = f"{label.lower()}_{name.replace(' ', '_')}"
                    if mapped_node_id not in seen_node_ids:
                        nodes.append(EvaluationNode(
                            id=mapped_node_id,
                            type=rf_type,
                            data={
                        ...
                        seen_node_ids.add(mapped_node_id)

                    edges.append(EvaluationEdge(
                        id=f"ev_e_{edge_counter}",
                        source=rule_node_id,
                        target=mapped_node_id,
                        data={"relationship": f"HAS_{label.upper()}"}
                    ))
                    edge_counter += 1
