...
function CountryGroupNodeInner({ data }: NodeProps) {
  ...
  return (
    <div className="bg-purple-50 border-2 border-purple-400 rounded-lg shadow-sm min-w-[200px] max-w-[220px]">
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-purple-600 text-xs font-bold">{expanded ? '▼' : '▶'}</span>
      ...
      {expanded && d.countries && d.countries.length > 0 && (
        <div className="border-t border-purple-200 px-3 py-1.5 max-h-[160px] overflow-y-auto">
          {d.countries.map((c) => (
